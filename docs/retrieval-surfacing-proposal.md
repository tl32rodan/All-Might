...
| **P-4** | **Claude Code mirror** = `PreToolUse` hook on `Grep`/`Glob` injecting `additionalContext` (DeusData proves this path). **Caution (2026-08-15):** `additionalContext` is *per-event*, not universal — it has no section for `PreCompact`/`PostCompact`, and the documented channel for `SessionStart`/`UserPromptSubmit` is plain stdout. Confirm `PreToolUse`'s own output section before building on it; see `docs/compaction-reprime-proposal.md` for the bug this caused. This requires a **new `PLATFORM_CAPABILITIES` key** (e.g. `pre_tool_inject: {opencode: True, claude_code: True}`) because the existing `tool_execute_after_inject` is marked `claude_code: False`. The plugin `requires` the new key so the manifest permits a mirror. | Mirrored via new capability key. | Medium — could ship OC-only first (`claude_code_mirror: None`) and promote later. |
