...
@dataclass(frozen=True)
class OrphanPersonality:
    """A personality directory on disk that is missing from the registry.

    ``capabilities`` lists capability template names whose data subdir
    exists under ``personalities/<name>/`` (e.g. ``database``,
    ``memory``). ``has_role_md`` records whether ``ROLE.md`` is present
    — a personality without one is treated as a non-actionable stray
    directory by the CLI even though we still surface it.
    """
...
def detect_orphan_personalities(project_root: Path) -> list[OrphanPersonality]:
    """Find personality dirs on disk that aren't in the registry.

    A directory ``personalities/<name>/`` is considered an orphan when
    its name is absent from ``.allmight/personalities.yaml``. The
    function returns every such dir — including ones missing
    ``ROLE.md`` or any capability subdir — so the caller (typically
    the ``allmight reconcile`` CLI) can decide how to present and
    filter them. Only the CLI / caller distinguishes actionable
    orphans from non-actionable strays.

    Capabilities are inferred from the presence of a subdirectory
    whose name matches a discovered capability template name. The
    per-personality ``commands/`` and ``skills/`` slots that
    ``compose`` always creates are deliberately *not* treated as
    capability markers (they exist for every personality).

    Returns an empty list when ``personalities/`` itself is missing —
    a fresh init with no personalities is a valid state, not an
    error.
    """
    ...
    template_names = {t.name for t in discover()}
    registered_names = {e.instance for e in read_registry(project_root)}

    orphans: list[OrphanPersonality] = []
    for child in sorted(personalities_dir.iterdir()):
        ...
        if child.name in registered_names:
            continue
        has_role = (child / "ROLE.md").is_file()
        detected = [
            cap for cap in sorted(template_names)
            if (child / cap).is_dir()
        ]
        orphans.append(OrphanPersonality(
            name=child.name,
            capabilities=detected,
            has_role_md=has_role,
        ))
    return orphans
...
def register_orphans(
    ...
    """Append orphan personalities to the registry and persist.

    Each orphan is recorded as a Part-D row with its detected
    capabilities and the *current* template version for each
    capability. Returns the resulting full registry list (existing +
    appended), in that order, so the caller can immediately recompose
    ``AGENTS.md`` and the per-personality OpenCode agent files without
    re-reading from disk.
    """
    template_by_name = {t.name: t for t in discover()}
    existing = read_registry(project_root)
    new_entries = list(existing)
    for orphan in orphans:
        new_entries.append(RegistryEntry(
            instance=orphan.name,
            capabilities=list(orphan.capabilities),
            versions={
                cap: template_by_name[cap].version
                for cap in orphan.capabilities
                if cap in template_by_name
            },
        ))
