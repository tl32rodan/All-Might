...
_AGENTS_MD_HEADER = (
    "<!-- all-might generated -->\n"
    "<!--\n"
    "  Everything between ALL-MIGHT:BEGIN and ALL-MIGHT:END is composed\n"
    "  from the framework primer plus each personality's ROLE.md, and is\n"
    "  rewritten on every `allmight init` / `add` / `compose`. Edit\n"
    "  personalities/<name>/ROLE.md to change a personality's behaviour.\n"
    "  Your own instructions belong OUTSIDE the fence — anything above or\n"
    "  below it is preserved byte-for-byte. To stop All-Might touching\n"
    "  this file at all, add an ALL-MIGHT:OFF html comment outside the\n"
    "  fence (same comment syntax as the fence markers themselves).\n"
    "-->\n\n"
    "# {project_name}\n"
)
...
def compose_agents_md(
    ...
    *,
    ...
    The composition is written **between the ALL-MIGHT fence markers**
    (see :mod:`allmight.core.agents_md`), never as the whole file.
    AGENTS.md is a shared surface: the user keeps their own project
    instructions and dedicated agent prompts in the same file, because
    it is the one file both OpenCode and (via the ``CLAUDE.md`` shim)
    Claude Code read automatically. Everything outside the fence is
    preserved byte-for-byte, whoever wrote it:

    * **No AGENTS.md** — the fenced block is the whole file.
    * **Hand-written AGENTS.md** (no fence, no All-Might content) — the
      user's bytes stay, the fenced block is appended below them.
    * **Fenced AGENTS.md** — only the fence interior is rewritten.
    * **Pre-fence All-Might output** — migrated in place; when the old
      content cannot be located exactly, the original file is copied to
      ``.allmight/templates/AGENTS.md.prev`` before rewriting so nothing
      is lost.
    * **``<!-- ALL-MIGHT:OFF -->`` present** — the file is not touched at
      all; the composition stages to ``.allmight/templates/AGENTS.md``
      for ``/sync``.

    Before this, the whole file was regenerated whenever it carried our
    marker — so a user who appended their own agent prompts to a
    previously-composed AGENTS.md lost them silently on the next
    ``allmight add``. The marker only proves "All-Might wrote this file
    once", never "the body is still ours".

    The returned path is whichever file was actually written.

    ``force=True`` still respects the fence (replacing its interior is
    already a complete overwrite of our share) but overrides the opt-out
    marker and replaces an unrecognisable file outright — used by the
    one-shot migrator on a legacy-format AGENTS.md known to be ours.
    """
    from .agents_md import removes_content, render_fenced, splice
    from .markers import ALLMIGHT_MARKER_MD
    ...
    existing = agents_md.read_text() if agents_md.is_file() else None
    merged, action = splice(existing, body, force=force)

    staged = project_root / ".allmight" / "templates" / "AGENTS.md"
    if action == "opted-out":
        staged.parent.mkdir(parents=True, exist_ok=True)
        staged.write_text(render_fenced(body))
        return staged

    if removes_content(action) and existing is not None:
        # Pre-fence content we could not place inside the fence. Keep a
        # recoverable copy next to the other /sync staging artifacts.
        backup = project_root / ".allmight" / "templates" / "AGENTS.md.prev"
    ...
    # A previously staged whole-file conflict is obsolete once the
    # composition lives in the real file — leaving it behind would send
    # /sync hunting for a conflict that no longer exists. Only our own
    # staging artifact is removed; AGENTS.md itself is never deleted.
    if staged.exists():
        staged.unlink()
    return agents_md
