...
**Only if you updated `memory/understanding/<workspace>.md` during
this `/remember` call** — that is, you wrote a new L2 entry or
Pattern Check above produced one (it may not have; check first) —
regenerate `memory/understanding/_index.md`. It is the table of contents
`/recall` reads first to decide which workspace file to load —
keeping it accurate is what makes per-workspace loading viable.
...
1. Read every `memory/understanding/*.md` in the active personality
   (skip `_index.md` itself).
2. For each, extract: the workspace name (basename without `.md`),
   the section count (count of `^## ` headings), and a one-line
   topic summary per section (the heading text itself is usually
   enough — keep ≤80 chars).
3. Write the regenerated `_index.md` following the schema above.
   Overwrite — do not append.

Keeping the index < ~500 tokens is the design budget; if your project
exceeds it, that is the signal to revisit the "no L2 RAG until 200
files" decision in `docs/plan.md` (D' non-goal).
