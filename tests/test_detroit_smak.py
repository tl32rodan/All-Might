...
class TestProjectInitializer:
    ...
    def test_creates_commands(self, sample_project):
        ...
        # Only /search is generated by the database capability — the
        # legacy /enrich and /ingest slash commands were retired.
        assert (commands_dir / "search.md").exists()
        assert not (commands_dir / "enrich.md").exists()
        assert not (commands_dir / "ingest.md").exists()
    ...
    def test_agents_md_has_getting_started(self, sample_project):
        """Test that AGENTS.md has getting started steps."""
        scanner = ProjectScanner()
        manifest = scanner.scan(sample_project)
    ...
    def test_agents_md_lists_search_only(self, sample_project):
        """AGENTS.md lists /search; retired commands stay out."""
        scanner = ProjectScanner()
        manifest = scanner.scan(sample_project)
    ...
    def test_no_enrich_command_in_sos_mode(self, sample_project):
        """``/enrich`` was retired — SOS mode does not bring it back."""
        scanner = ProjectScanner()
        manifest = scanner.scan(sample_project)
        ...
        assert not (
            sample_project / ".opencode" / "commands" / "enrich.md"
        ).exists()

    def test_no_ingest_command_in_sos_mode(self, sample_project):
        """``/ingest`` was retired — SOS mode does not bring it back."""
        scanner = ProjectScanner()
        manifest = scanner.scan(sample_project)
        manifest.has_path_env = True
        ...
        assert not (
            sample_project / ".opencode" / "commands" / "ingest.md"
        ).exists()
