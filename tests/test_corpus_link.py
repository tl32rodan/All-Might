...
# ======================================================================
# Command Content (generated by initializer)
# ======================================================================


class TestCorpusCommands:
    """Corpus commands are generated as .claude/commands/ operational guides."""
    ...
    def test_corpus_link_command_generated(self, project_root):
        assert (project_root / ".claude" / "commands" / "corpus-link.md").exists()

    def test_corpus_unlink_command_generated(self, project_root):
        assert (project_root / ".claude" / "commands" / "corpus-unlink.md").exists()

    def test_corpus_list_command_generated(self, project_root):
        assert (project_root / ".claude" / "commands" / "corpus-list.md").exists()

    def test_corpus_link_has_how_to(self, project_root):
        content = (project_root / ".claude" / "commands" / "corpus-link.md").read_text()
        assert "How to execute" in content
        assert "ln -s" in content
        assert ".links.yaml" in content

    def test_corpus_unlink_has_how_to(self, project_root):
        content = (project_root / ".claude" / "commands" / "corpus-unlink.md").read_text()
        assert "How to execute" in content
        assert "readlink" in content

    def test_corpus_list_has_how_to(self, project_root):
        content = (project_root / ".claude" / "commands" / "corpus-list.md").read_text()
        assert "How to execute" in content
        assert ".links.yaml" in content

    def test_claude_md_lists_corpus_commands(self, project_root):
        content = (project_root / "CLAUDE.md").read_text()
        assert "/corpus-link" in content
        assert "/corpus-unlink" in content
        assert "/corpus-list" in content
